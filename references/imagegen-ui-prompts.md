...
Write prompts around the product and a deliberate visual proposition, not a reference or a component checklist.
...
4. **Reference scope**: attach the anchor image and name the visual dimensions it may influence; forbid copying its layout, structure, components, content, and identity.
...
Do not name Mobbin source products inside the prompt. The attachment should transfer visual craft, not product semantics. Keep exclusions short and specific. If the same unwanted element survives twice, describe the positive alternative rather than extending a negative list.
